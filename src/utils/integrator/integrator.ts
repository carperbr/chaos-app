...
export class Integrator {
  ...
  estimate4th: number = 0;
  estimate5th: number = 0;
  buffer: number[];
  state: number[] = [];
  midpoints: number[];
  parameters: number[] = [];
  ...
  step() {
    ...
    for (let stage = 0; stage < 7; stage++) {
      ...
      if (stage < 6) {
        for (let i = 0; i < this.state.length; i++) {
          this.midpoints[i] =
            this.state[i] +
            this.getWeight(0, stage) * this.getBuffer(0, i) +
            this.getWeight(1, stage) * this.getBuffer(1, i) +
            this.getWeight(2, stage) * this.getBuffer(2, i) +
            this.getWeight(3, stage) * this.getBuffer(3, i) +
            this.getWeight(4, stage) * this.getBuffer(4, i) +
            this.getWeight(5, stage) * this.getBuffer(5, i);
        }
      }
    }
    ...
    for (let i = 0; i < this.state.length; i++) {
      this.estimate4th =
        this.state[i] +
        this.getWeight(0, 6) * this.getBuffer(0, i) +
        this.getWeight(1, 6) * this.getBuffer(1, i) +
        this.getWeight(2, 6) * this.getBuffer(2, i) +
        this.getWeight(3, 6) * this.getBuffer(3, i) +
        this.getWeight(4, 6) * this.getBuffer(4, i) +
        this.getWeight(5, 6) * this.getBuffer(5, i);
      this.estimate5th =
        this.state[i] +
        this.getWeight(0, 7) * this.getBuffer(0, i) +
        this.getWeight(1, 7) * this.getBuffer(1, i) +
        this.getWeight(2, 7) * this.getBuffer(2, i) +
        this.getWeight(3, 7) * this.getBuffer(3, i) +
        this.getWeight(4, 7) * this.getBuffer(4, i) +
        this.getWeight(5, 7) * this.getBuffer(5, i) +
        this.getWeight(6, 7) * this.getBuffer(6, i);
      this.state[i] = this.estimate5th;
      this.error = Math.abs(this.estimate5th - this.estimate4th);
      this.dtOpt =
        0.75 * this.dt * Math.min(Math.max(this.epsilon / this.error, 0.1), 2);
